...
icu_instance_ctrl_t g_int_gt911_ctrl;
const external_irq_cfg_t g_int_gt911_cfg =
{ .channel = 3, .trigger = EXTERNAL_IRQ_TRIG_RISING, .filter_enable = false, .clock_source_div =
          EXTERNAL_IRQ_CLOCK_SOURCE_DIV_64,
  .p_callback = touch_int_callback,
...
const external_irq_instance_t g_int_gt911 =
{ .p_ctrl = &g_int_gt911_ctrl, .p_cfg = &g_int_gt911_cfg, .p_api = &g_external_irq_on_icu };
...
{ .pll_settings = /* Calculated MIPI PHY PLL frequency: 1000000000 Hz (error 0.00%) = 20000000 Hz / 1 * 50.00 */
{ .div = 1 - 1, .mul_int = 50 - 1, .mul_frac = 0 /* Value: 0 */},
...
jpeg_instance_t const * _g_jpeg_emwin_decode = &RA_NOT_DEFINED;
#endif
#endif
...
SemaphoreHandle_t g_semaphore_gt911_irq;
#if 1
StaticSemaphore_t g_semaphore_gt911_irq_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_semaphore_gt911_i2c;
#if 1
StaticSemaphore_t g_semaphore_gt911_i2c_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_timer_semaphore;
#if 1
StaticSemaphore_t g_timer_semaphore_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_mipi_semaphore;
#if 1
StaticSemaphore_t g_mipi_semaphore_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
SemaphoreHandle_t g_touch_reset_semaphore;
#if 1
StaticSemaphore_t g_touch_reset_semaphore_memory;
#endif
void rtos_startup_err_callback(void *p_instance, void *p_data);
void g_common_init(void)
{
    g_semaphore_gt911_irq =
#if 1
            xSemaphoreCreateBinaryStatic (&g_semaphore_gt911_irq_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_semaphore_gt911_irq)
    {
        rtos_startup_err_callback (g_semaphore_gt911_irq, 0);
    }
    g_semaphore_gt911_i2c =
#if 1
            xSemaphoreCreateBinaryStatic (&g_semaphore_gt911_i2c_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_semaphore_gt911_i2c)
    {
        rtos_startup_err_callback (g_semaphore_gt911_i2c, 0);
    }
    g_timer_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_timer_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_timer_semaphore)
    {
        rtos_startup_err_callback (g_timer_semaphore, 0);
    }
    g_mipi_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_mipi_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_mipi_semaphore)
    {
        rtos_startup_err_callback (g_mipi_semaphore, 0);
    }
    g_touch_reset_semaphore =
#if 1
            xSemaphoreCreateBinaryStatic (&g_touch_reset_semaphore_memory);
#else
                xSemaphoreCreateBinary();
                #endif
    if (NULL == g_touch_reset_semaphore)
    {
        rtos_startup_err_callback (g_touch_reset_semaphore, 0);
    }
}
