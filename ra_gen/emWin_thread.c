...
static StaticTask_t emWin_thread_memory;
                ...
                static uint8_t emWin_thread_stack[0x1000] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
static uint8_t emWin_thread_stack[0x1000] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.emWin_thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
#endif
#endif
...
void emWin_thread_create(void)
{
    ...
    emWin_thread = xTaskCreateStatic (
                    ...
                    #endif
                                      emWin_thread_func,
                                      (const char*) "emWin Thread", 0x1000 / 4, // In words, not bytes
